...
public final class GlobalRankedAd{
    ...
    public void derank(boolean errored) {
        ...
        logger.fine("Deranking bid: "+bid+" hard="+errored+" total deranks: "+derankEvents.size()+" at time: "+currentTime);
    }
    ...
    private double calculateDerankFactor() {
        ...
        for(int i=0;i<derankEvents.size();i++){
            ...
            double currentImpact=INITIAL_DERANK_IMPACT+(MINIMUM_DERANK_IMPACT-INITIAL_DERANK_IMPACT)*(1.0-Math.exp(-1.5*timeDecayFactor));
        ...
        logger.fine("Derank calculation for bid "+bid.getId()+": activeDeranks="+derankEvents.size()+", totalImpact="+totalDerankImpact+", finalFactor="+derankFactor);
    ...
    protected double getBaseScore() {
        ...
        logger.fine("Base score calculation for bid "+bid.getId()+": price="+priceScore+", derankFactor="+derankFactor+", penaltyFactor="+penaltyFactor+", final="+finalScore);
    ...
    public double getContextualScore(Adspace space, int width, int height) {
        ...
        double sizeScore=Math.sqrt(Math.min(width,bidW)/(double)Math.max(width,bidW)*Math.min(height,bidH)/(double)Math.max(height,bidH));
