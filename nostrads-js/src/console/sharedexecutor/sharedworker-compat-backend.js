    ...
    async update() {
        ...
        for (const [id, invokation] of Object.entries(this.invokations)) {
            if (this.self.isMaster) {
                ...
                try {
                    const f = this.registeredMethods[invokation.method];
                    if(!f){
                        throw new Error(`Method ${invokation.method} is not registered.`);
                    }
                    const p = f(...invokation.args);

                    if (p && typeof p.then === 'function') {
                        p.then(res => {
                            invokation.res(res);
                        }).catch(err => {
                            console.error(`Error in invokation ${id} for method ${invokation.method}:`, err);
                            invokation.rej(err);
                        });
                    } else {
                        invokation.res(p);
                    ...
                    invokation.rej("Error calling method " + invokation.method+": "+error);
                }
