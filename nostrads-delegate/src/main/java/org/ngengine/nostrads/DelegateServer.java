...
public class DelegateServer{
    ...
    private static final List<String> DEFAULT_RELAYS = List.of("wss://relay.ngengine.org","wss://relay2.ngengine.org","wss://relay.damus.io","wss://relay.primal.net","wss://relay.nostr.band");
    ...
    private static void load() throws Exception {
        ...
        DelegateService service=new DelegateService(pool,new NostrKeyPairSigner(keyPair),taxonomy,filterOffers,filterBids,new PenaltyStorage(penaltyStore),new Tracker(trackerStore));
        String collectorLnAddress=(String)config.getOrDefault("feeCollectorLnAddress",null);
        service.setFee((long)config.getOrDefault("minFeeMsats",0L),(double)config.getOrDefault("percentFee",0.01),(long)config.getOrDefault("maxFeeMsats",10000L),collectorLnAddress==null?null:new LnAddress(collectorLnAddress));
        ...
        if(collectorLnAddress!=null){
            userMetadata.metadata.put("nostrads:fees",String.format("%d:%f:%d",config.getOrDefault("minFeeMsats",0L),config.getOrDefault("percentFee",0),config.getOrDefault("maxFeeMsats",10000L)));
        }else{
        ...
        System.out.println("Fee: "+config.getOrDefault("percentFee",0)+"% (min "+config.getOrDefault("minFeeMsats",0L)+" msats, max "+config.getOrDefault("maxFeeMsats",10000L)+" msats)");
        if(collectorLnAddress!=null){
        ...
        for(int i=0;i<args.length;i++){
            ...
            try{
                switch(type){
                    ...
                    case "--disallowBidder":{
                        List<String> biddersBlacklist=(List<String>)config.computeIfAbsent("biddersBlacklist",(r) -> new ArrayList<String>());
                        biddersBlacklist.add(args[++i]);
                        break;
                    }
                    case "--allowBidder":{
                        List<String> biddersWhitelist=(List<String>)config.computeIfAbsent("biddersWhitelist",(r) -> new ArrayList<String>());
                        biddersWhitelist.add(args[++i]);
                        break;
                    }
                    case "--disallowOfferer":{
                        List<String> offerersBlacklist=(List<String>)config.computeIfAbsent("offerersBlacklist",(r) -> new ArrayList<String>());
                        offerersBlacklist.add(args[++i]);
                        break;
                    }
                    case "--allowOfferer":{
                        List<String> offerersWhitelist=(List<String>)config.computeIfAbsent("offerersWhitelist",(r) -> new ArrayList<String>());
                    ...
                    case "--fee":{
                        ...
                        if(parts.length!=4){ throw new IllegalArgumentException("Invalid fee format, expected minFeeMsats:percentFee:maxFeeMsats:collectorLnAddress"); }
                    ...
                    case "--config":{
                        ...
                        try{
                            try(BufferedInputStream fis=new BufferedInputStream(new FileInputStream(new File(configFile)))){
                    ...
                    case "--help":{
                        ...
                        System.out.println("  --id                         Set the ID of the delegate server (default is name with spaces replaced by dashes)");
                        System.out.println("  --fee <minFeeMsats:percentFee:maxFeeMsats:collectorLnAddress> Set the fee structure. eg. 2000:0.05:10000:nostr4j@ln.rblb.it");
